...
from iris.augmentations.instruction_augmentations.jailbreaks.utils.gpt_fuzzer.template import QUESTION_PLACEHOLDER


class Mutator:
    def __init__(self, fuzzer: 'GPTFuzzerJailbreaking'):
        self._fuzzer = fuzzer
        self.n = None
...
class OpenAIMutatorBase(Mutator):
    def __init__(self,
                 ...
                 fuzzer: 'GPTFuzzerJailbreaking' = None):
    ...
    def mutate_single(self, seed) -> List[str]:
        return [self.model.generate(seed, temperature=self.temperature, max_tokens=self.max_tokens, n=self.n, max_trials=self.max_trials, failure_sleep_time=self.failure_sleep_time)]
...
class MutateRandomSinglePolicy(MutatePolicy):
    ...
    def mutate_single(self, prompt_node: 'PromptNode') -> List['PromptNode']:
        ...
        if self.concatentate:
            results = results + [prompt_node.prompt]
