...
class SelectPolicy:
    ...
    def update(self, questions: List[str], prompt_nodes: List['PromptNode']):
        pass


class RoundRobinSelectPolicy(SelectPolicy):
    def __init__(self, fuzzer: 'GPTFuzzerJailbreaking' = None):
...
class UCBSelectPolicy(SelectPolicy):
    ...
    def update(self, questions: List[str], prompt_nodes: List['PromptNode']):
        ...
        self.rewards[self.last_choice_index] += \
            succ_num / len(questions)


class MCTSExploreSelectPolicy(SelectPolicy):
    def __init__(self, fuzzer: 'GPTFuzzerJailbreaking' = None, ratio=0.5, alpha=0.1, beta=0.2):
        super().__init__(fuzzer)
    ...
    def update(self, questions: List[str], prompt_nodes: List['PromptNode']):
        ...
        for prompt_node in reversed(self.mctc_select_path):
            reward = succ_num / (len(questions) * len(prompt_nodes))
