...
class ICAJailbreaking(Jailbreaking):
    def __init__(
        self, 
        target_model: ModelBase, 
        evaluator: Callable = None,
    ...
    def _attack(self, instruction: str, reference_answers: List[str] = None) -> List[Tuple[str, str]]:
        ...
        init_prompt = self.seed_template(seeds_num=1, prompt_usage='attack', methods=['ICA'], template_file=template_file)
...
if __name__ == "__main__":
    ...
    attacker = ICAJailbreaking(
        target_model=target_model, 
        evaluator=evaluator,
    )

    jailbreaked_samples = attacker.augment_batch(harmful_samples)
    print(f"ASR on Harmful prompts with ICAJailbreaking: {attacker.attack_success_rate}")
    ...
    jailbreaked_samples = attacker.augment_batch(harmful_samples)
    print(f"ASR on Benign prompts with ICAJailbreaking: {attacker.attack_success_rate}")