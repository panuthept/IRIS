...
class LogitLens:
    def __init__(
        ...
        self.cached_attentions: List[Tensor] = [] # list of Tensor of shape (num_layers, seq_len)
    ...
    def cache_attentions(self, attentions: Tuple[Tensor], tokens: Tensor):
        ...
        Output
            cached_attentions: Tensor of shape (num_layers, seq_len)
        """
        attentions = [attention[:, :, -1] for attention in attentions] # Only save the last token attentions
        attentions = torch.stack(attentions, dim=1) # shape (num_samples, num_layers, num_heads, seq_len)
        attentions = attentions.mean(dim=2) # shape (num_samples, num_layers, seq_len)
        attentions = attentions.squeeze(0) # shape (num_layers, seq_len)
