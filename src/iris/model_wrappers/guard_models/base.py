...
class GuardLLM(LLM):
    ...
    def complete(self, instruction: str, **kwargs) -> Dict[str, List[Tuple[str, float, float]]]:
        ...
        return {
            "pred_labels": [list(zip(labels, labels_probs, labels_logits))[:len(self.valid_tokens)] for labels, labels_probs, labels_logits in zip(lst_labels, lst_labels_probs, lst_labels_logits)],
            "pred_tokens": [list(zip(tokens, tokens_probs, tokens_logits))[:len(self.valid_tokens)] for tokens, tokens_probs, tokens_logits in zip(lst_tokens, lst_tokens_probs, lst_tokens_logits)],
