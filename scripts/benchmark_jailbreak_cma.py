...
if __name__ == "__main__":
    ...
    parser.add_argument("--do_cma", action="store_true")
    ...
    if not args.eval_only:
        if args.api_key:
            ...
            if args.intervention:
                ...
                # Todo: 
                # 1. Modify activation name
                # 2. Modify internvetion layers?
                # 3. Compute metrics for each sample and compile all (mean+std)
                # metrics: (after_logits - before_logits) / (ref_logits - before_logits)
